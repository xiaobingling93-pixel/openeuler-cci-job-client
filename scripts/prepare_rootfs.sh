...
# 同步内核文件并创建软链接
rsync -avz "${LOCAL_ROOTFS_DIR}/vmlinuz" root@${TARGET_IP}:${OS_BASE_DIR}/${TIMESTAMP}/boot/vmlinuz-${TIMESTAMP}  || error_exit "rsync vmlinuz failed"
...
# 同步模块文件并创建软链接
rsync -avz "${LOCAL_ROOTFS_DIR}/modules.cgz" root@${TARGET_IP}:${OS_BASE_DIR}/${TIMESTAMP}/boot/modules-${TIMESTAMP}.cgz || error_exit "rsync modules.cgz failed"
...
# 同步rootfs文件并改名
rsync -avz "${LOCAL_ROOTFS_DIR}/rootfs.cgz" root@${TARGET_IP}:${INITRD_BASE_DIR}/${TIMESTAMP}/current || error_exit "rsync rootfs.cgz failed"
# 复制ipconfig文件
